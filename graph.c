/********************************************************
 * Name: Sam Jones jonesamu
 * Date: 8/11/201
 * Description: implementations of dfs and bfs based on code provided by instructor
 *
 *******************************************************/
...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	cirListDeque *search_stack = malloc(sizeof(cirListDeque));
	initCirListDeque(search_stack);

	addBackCirListDeque(search_stack, source);

	while (!isEmptyCirListDeque(search_stack))
	{
		Vertex* currentVert = backCirListDeque(search_stack);
		removeBackCirListDeque(search_stack);
		...
		for (int i = 0; i < currentVert->numNeighbors; i++)
		{
			if (!(currentVert->neighbors[i]->isVisited))
				addBackCirListDeque(search_stack, currentVert->neighbors[i]);
		}
	}
	clearVisited(g);
	removeAllCirListDeque(search_stack);
	free(search_stack);
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	cirListDeque *search_queue = malloc(sizeof(cirListDeque));
	initCirListDeque(search_queue);

	addBackCirListDeque(search_queue, source);

	while (!isEmptyCirListDeque(search_queue))
	{
		Vertex* currentVert = frontCirListDeque(search_queue);
		removeFrontCirListDeque(search_queue);
		currentVert->isVisited = 1;
		if (currentVert == destination)
			return 1;
		...
		{
			if (!(currentVert->neighbors[i]->isVisited))
				addBackCirListDeque(search_queue, currentVert->neighbors[i]);
		}
	}
	clearVisited(g);
	removeAllCirListDeque(search_queue);
	free(search_queue);
	return 0;
}